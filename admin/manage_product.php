<?php
ob_start(); // Line 1 (Add this)
session_start(); // Line 2 (Make sure sessions start before output)
require('top.inc.php');
...
if(isset($_POST['submit'])){
	...
	if($msg==''){
		if(isset($_GET['id']) && $_GET['id']!=''){
			...
			$image=rand(111111111,999999999).'_'.$_FILES['image']['name'];
			$upload_dir = "X:/new_xampp/htdocs/php/ecom/media/product/"; // Line 110

if (!is_dir($upload_dir)) { 
    mkdir($upload_dir, 0777, true); // Line 111 (create folder if missing)
}

$upload_path = $upload_dir . $image; // Line 112

if (!move_uploaded_file($_FILES['image']['tmp_name'], $upload_path)) { // Line 113
    die("File upload failed! Check folder permissions."); // Line 114
}
							...
							<div class="card-body card-block">
							   <div class="form-group">
								...
								<div class="form-group">
									...
									<textarea name="description" placeholder="Enter product description" class="form-control" required><?php echo nl2br($description);?></textarea>
								</div>
