...
function productSoldQtyByProductId($con,$pid){
	...
	// and order_detail.product_id=$pid and 'order'.order_status!=4 and (('order'.payment_type='payu' and 
	// 'order'.payment_status='Success') or ('order'.payment_type='COD' and 'order'.payment_status!=''))";
	...
	AND (
		(`order`.payment_type = 'payu' AND `order`.payment_status = 'Success') 
	...
	// if (!$res) {
	// 	die("SQL Error: " . mysqli_error($con)); // Show exact SQL error
	//     }
	...
	$res = mysqli_query($con, $sql);
	if (!$res) {
		die("SQL Error: " . mysqli_error($con) . " in query: " . $sql);
	}
	$row = mysqli_fetch_assoc($res);
